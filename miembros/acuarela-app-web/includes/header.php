...
<aside>
    ...
    <div class="image">
        <?=
        isset($_SESSION["userAll"]->photo) && isset($_SESSION["userAll"]->photo->url) ? '<img src="https://acuarelacore.com/api/'.$_SESSION["userAll"]->photo->url.'" alt="profilePhoto" />' : '<img src="img/placeholder.png" alt="profilePhoto" />'?>
    </div>
    <div class="profile__txt">
        <h2><?=$_SESSION["userAll"]->name?> <?=$_SESSION["userAll"]->lastname?></h2>
        <p><?=$_SESSION["userAll"]->acuarelauser->rols[0]->rol?></p>
        <p>Daycare: <strong id="daycareName"><?=isset($foundDaycare) && isset($foundDaycare->name) ? $foundDaycare->name : (isset($_SESSION["user"]->daycares[0]) && isset($_SESSION["user"]->daycares[0]->name) ? $_SESSION["user"]->daycares[0]->name : 'No disponible')?></strong></p>
    </div>
    </div>
    <nav>
    <a href="/miembros/acuarela-app-web/" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 0 ? 'active':''?>"><i class="acuarela acuarela-Anadir_reaccion"></i>Social</a>
    <a href="/miembros/acuarela-app-web/inscripciones" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 1 ? 'active':''?>" ><i class="acuarela acuarela-Formulario"></i><span data-translate="7">Agregar niñxs</span></a>
    <a href="/miembros/acuarela-app-web/asistencia" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 2 ? 'active':''?>"><i class="acuarela acuarela-Asistencia"></i><span data-translate="8">Asistencia</span></a>
    <a href="/miembros/acuarela-app-web/asistentes" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 3 ? 'active':''?>"><i class="acuarela acuarela-Asistente"></i><span data-translate="9">Asistentes</span></a>
    <a href="/miembros/acuarela-app-web/grupos" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 4 ? 'active':''?>"><i class="acuarela acuarela-Grupo"></i><span data-translate="60">Grupos</span></a>
    <a href="/miembros/acuarela-app-web/administrador-tareas" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 9 ? 'active':''?>"><i class="acuarela acuarela-Checklist"></i><span data-translate="193">Administrador de tareas</span></a>
    <?php 
    ...
    <a href="/miembros/acuarela-app-web/finanzas" class="<?=isset($_GET['activeTab']) && $_GET['activeTab'] == 5 ? 'active':''?>"><i class="acuarela acuarela-Finanzas"></i><span data-translate="53">Finanzas</span></a>
    <?php }else{ ?>
