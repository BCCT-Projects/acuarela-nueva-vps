...
| :--- | :--- | :--- |
| **Identidad del Menor** | `name`, `lastname`, `birthday` | Identidad completa protegida (Nombres + Fecha Nacimiento). Garantiza anonimato total en BD. |
| **Salud del Menor** | `healthinfos` (Alergias, medicamentos, incidentes, etc.) | Información médica sensible (HIPAA compliant approach). |
| **Contacto Padres** | `phone`, `work_phone` | Dato de contacto directo primario y laboral. |
...
| **Datos de Pediatra** | **Validación y Naturaleza Pública:** El email del pediatra requiere validación de formato estricta en base de datos. Además, son datos de contacto profesional (Tarjetas de presentación), no PII confidencial del menor. | Se almacenan en texto plano (`pediatrician_email`, `pediatrician_number`). |
